...
function draw()
{
  ...
  {
    text("Kill 50 Zombies to lure the boss zombie.You have 50 bullets to do so.", 80, 25);
    text("Once you find the boss zombie, kill it and escape with the helicopter in the given time.", 80, 75);
    text("TIP : Aim at the Zombie's head." , 80, 125);
    text("Press UP ARROW to start." , 80, 175);
  ...
  if(gameState === "PLAY")
  {
    ...
    {
      var currentZombie = zombieGroup.get(0);
      if((player.y === currentZombie.y) || 
        (player.scale < initialScale && player.y >= currentZombie.y) ||
        (player.scale === initialScale && player.y <= currentZombie.y))
      {
        currentZombie.destroy();
        life--;
      }
    }
